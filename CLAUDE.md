...
- **O gate do `.env` foi cego por meses — e a lição é sobre o formato do gate, não sobre a chave.** Em 02/08/2026 o `.env` carregava a chave **SECRETA** do Supabase (`sb_secret_…`, a que ignora RLS) dentro da variável `SUPABASE_ANON_KEY`, e o `check_env_safety.sh` passava **verde**. Ele só sabia procurar (a) NOMES de variável casando `(OPENAI|SERVICE_ROLE|SECRET|…)` — e "SUPABASE_ANON_KEY" não casa com nada — e (b) JWTs `eyJ…` decodificados atrás de `role=service_role` — e as chaves novas do Supabase são **opacas, não são JWT**. O segredo passava pelo vão entre as duas regras. Correções aplicadas: checagem por **VALOR** (prefixos `sb_secret_`/`sk-`/`sk_live_`/`re_`/`apify_api_`), **asserção positiva** de que `SUPABASE_ANON_KEY` é `sb_publishable_…` ou JWT `role=anon` (allowlist pega formato futuro que denylist não pegaria), varredura de `sb_secret_` em arquivo rastreado, e `.gitignore` de `.env` → `.env*` + `!.env.example` (um backup `.env.bak` ficava untracked e commitável). **Regra geral: gate de segredo tem que validar o VALOR e ter pelo menos uma asserção positiva — denylist por nome sempre atrasa em relação a formato novo.** Ao mexer no script, mutation-teste: injete a chave errada e confirme que ele FALHA (verde não prova nada).
- **Trocar a chave do app APLICA a RLS que estava dormindo.** Corolário do item acima e a parte que quase passou batido: com a chave secreta, o cliente lia tudo ignorando policy. Com a publicável, `app_feature_flags` (policy só para `authenticated`) devolve `[]` no cold start de quem não tem sessão — e `FeatureFlagsService.refresh()` tem call site único em `main.dart:151`, dentro do `_bootstrap()`, antes do login e num `try/catch(_){}`. Instalação nova passaria a primeira sessão inteira com os 16 call sites em v1, sem erro e sem log. Migration `20260802120000` libera a leitura para `public`. **Antes de trocar qualquer chave, liste o que dependia do bypass de RLS.**
...
**Ajustes pré-2.4.0 (15/06, branch `fase-2-fixes` de main; `FASE-2-FIXES-RELATORIO.md`)** — 4 defeitos da validação device, saem JUNTOS na 2.4.0: #3 detalhe da lista/salvas mostrava 0% (abria sem match) → cache de resultado movido pro `JobsViewModel` (compartilhado swipe↔detalhe) + `resolveMatchForJob` + pending=spinner; #1 célula da lista só com razões (banda saiu — divergia do detalhe; ordenação segue por rank_score); #2 copy do detalhe com baldes 85/70/40/<40; #4 `inferArea` com 2 rulesets (título completo, descrição forte — sem boilerplate) + backfill `tools/reclassify_active_areas/`. Commits `07c5185`+`4a97c3b`+`035ab1c` (fix-forward de regressões de saúde pegas no dry-run do backfill). Verde: analyze 0/0, flutter test 40, deno test 11/11, deno check 27. **Deploy+backfill FEITOS (15/06):** as 4 functions que embarcam `_shared/jobs.ts` deployadas (2 rodadas), `check_functions_drift` OK; backfill de 57 vagas gupy/brz aplicado via MCP (backup local revertível). Aceites em prod: **Tech-sem-token-no-título 16→1**, regressão de saúde 0, **paridade feed_parity 7/7**. **+ Fix #5 (`ab95d2e`):** exaustão do feed mostrava "filtros restritivos" (B) em vez de "esgotou as relevantes" (A) após swipar tudo — `filtersAreTooRestrictive` usava totais pós-swipe; agora usa `totalMatchingCatalog` (matches ignorando swipe), via função pura `feedFiltersTooRestrictive`. Follow-up RPC FEITO (`5901461`): `get_feed_page` **v1.3** (migration `20260615120000`) retorna `total_matching_catalog` (matches ignorando swipe) — base inclui swipadas + distintos do catálogo inteiro; client lê do FeedPager. feed_parity 7/7 (feed inalterado), perf 13,8/4,5/8,9ms. Tela de exaustão (swipe) também virou rolável (`6d88dfc`, overflow no estado A). **Falta (fundador):** só validação device. Detalhe: `FASE-2-FIXES-RELATORIO.md`.

**Fase 3 EXECUTADA (16/06, branch `fase-3-tracker` de main, 8 commits; `PLANO-FASE-3.md` aprovado + `FASE-3-RELATORIO.md`)** — tracker de candidaturas + prompt de retorno + saída instrumentada. Achado-chave (o fato vence): **um único call site de apply** (`liked_jobs_screen.dart::_openApplication`) — o `JobDetailsSheet` não tem botão de aplicar (auditoria H1/PLANO-MÃE "dois call sites" desatualizada); apply é downstream dos dois feeds → cobre swipe E lista por construção. Reuso das constantes mortas `job_details_apply_external_opened`/`job_details_apply_returned` (zera dívida R7) e de `application_state_changed` (carrega `application_type`) para status manual. **SERVER-IMEDIATO em prod:** T3.4 (`outbound_clicks` RLS own + `applications.external_url`, `FASE3_PR1_TESTS_OK`) + T3.5 (digest 2ª passada de prazo ≤48h, intent `saved_deadline_48h` sem push novo; índice `jobs_deadline_active_idx` MEDIDO — EXPLAIN 303ms→33.8ms range scan; dedupe smoke ao vivo = 1 push). **CLIENT na release 2.5.0 atrás da flag `applications_tracker_v1` (OFF/0%):** T3.1 aba Candidaturas (4 segmentos + status menu) + T3.2 prompt (pending_apply + observer HomeScreen + sheet Sim/Não/Depois + 4 chips de abandono) + T3.3 adição manual (FAB). Decisões do fundador: 4 segmentos, 4 chips fixos, re-pergunta in-app 24h (sem push). **Aceite ≥30% conta da ATIVAÇÃO da flag** (gate no prompt). Verde: flutter test 77, analyze 0/46, deno test 6/6, deno check 27, drift OK (25, repo==deployado), migration list limpo (96). **Falta (fundador):** PR/merge, release 2.5.0, ativar a flag (rollout), validação device. Conta interna em prod já é `phone_5500900000001@stage.app` (T2.0 aparenta aplicado).
