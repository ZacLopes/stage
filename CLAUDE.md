...
**Fase 3 EM EXECUÇÃO (16/06, branch `fase-3-tracker` de main; `PLANO-FASE-3.md` aprovado pelo fundador)** — tracker de candidaturas + prompt de retorno + saída instrumentada. Achado-chave (o fato vence): **um único call site de apply** (`liked_jobs_screen.dart::_openApplication`) — o `JobDetailsSheet` não tem botão de aplicar (a auditoria H1/PLANO-MÃE "dois call sites" estava desatualizada); apply é downstream dos dois feeds → cobre swipe E lista por construção. Reuso das constantes mortas `job_details_apply_external_opened`/`job_details_apply_returned` (zera dívida R7) e de `application_state_changed` (carrega `application_type`) para status manual. **Camadas:** T3.4 (`outbound_clicks` + `applications.external_url`) e T3.5 (digest com prazos ≤48h, índice `jobs_deadline_active_idx` MEDIDO via EXPLAIN — 303ms→range scan) são SERVER-IMEDIATO (PR1/PR2); T3.1 aba Candidaturas (4 segmentos) + T3.2 prompt + T3.3 adição manual saem na **release 2.5.0** atrás da flag `applications_tracker_v1` (rollout 10→50→100). Decisões do fundador: 4 segmentos (Entrevistas só na F4), 4 chips de abandono fixos, re-pergunta in-app 24h (sem push — F3 proíbe push novo). Aceite ≥30% conta da ATIVAÇÃO da flag, não da release. **e2e device da F3 depende do T2.0 da F2** (conta interna por telefone) ainda não rodado.
