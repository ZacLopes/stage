...
void main() {
  ...
  testWidgets('40–69 → "Match parcial"', (tester) async {
    await pumpWithScore(tester, 50);
    expect(find.text('Match parcial'), findsOneWidget);
  });

  testWidgets('70–84 → "Bom match"', (tester) async {
    await pumpWithScore(tester, 75);
    expect(find.text('Bom match'), findsOneWidget);
  });

  testWidgets('>= 85 → "Excelente match"', (tester) async {
    await pumpWithScore(tester, 90);
    expect(find.text('Excelente match'), findsOneWidget);
  });
}
