-- FASE 2 (T2.1) — get_feed_page v1.1: correção de performance, MESMA semântica.
--
-- A verificação de performance do T2.1 (re-EXPLAIN pós-deploy, regra do plano:
-- "regressão vs plan mode = parar e investigar") flagrou o perfil (c) com
-- ~167-175ms warm (B1 projetava 5-21ms na forma final): os EXISTS de
-- área-com-sinônimos e cidade↔UF viravam SUBPLANS CORRELACIONADOS no plano,
-- re-executados POR VAGA (417 loops × ~0,13ms × 2 colunas pass_/m_ de área
-- + 410 loops × 2 de cidade ≈ 170ms) — e o custo dobraria a cada milhar de
-- vagas, furando a projeção de 5k do §3.3.
--
-- Correção: os mesmos predicados, avaliados 1x POR VALOR DISTINTO em CTEs
-- de lookup keyed pelo valor CRU (jobs.area tem ≤12+test valores distintos;
-- (city,state) ~dezenas) — o per-row vira hash lookup não-correlacionado
-- (`in (select …)` hashed). unaccent roda ~130x no total, INDEPENDENTE do
-- número de vagas (antes: O(vagas × sinônimos)). Pós-fix medido (warm):
-- perfil (a) 9ms · (b) 4-6ms · (c) 20-21ms (era 167-175ms);
-- 7/7 paridade re-verificada; FASE2_TESTS_OK re-rodado.
--
-- Paridade by construction: cada predicado é função pura do (valor cru,
-- args) — avaliá-lo por valor distinto ou por row é matematicamente igual.
-- Espelho de filter_helpers.dart inalterado (sinônimos/cidades idênticos ao
-- 20260612120000). Mudou lá → muda aqui → roda tools/feed_parity/.
...
begin
  ...
  -- prefs de ranking; check de vazio CRU (espelho do isNotEmpty do client)
  ...
  -- lookups por valor distinto: predicados IDÊNTICOS aos do v1.0, avaliados
  -- 1x por área / combo (cidade,estado) em vez de 1x por vaga
  ...
  scored as (
    select b.id, b.published_at,
      ...
      -- m_* TODOS gateados pela dimensão declarada (numerador ⊆ denominador)
...
comment on function public.get_feed_page is
  'Fase 2 v1.1: feed paginado por keyset (rank_score DESC, id DESC), ranking determinístico v1; matching por valor distinto (perf). Espelho de filter_helpers.dart — mudou lá, roda tools/feed_parity/.';

-- CREATE OR REPLACE preserva ACLs, mas repete por auto-contenção:
