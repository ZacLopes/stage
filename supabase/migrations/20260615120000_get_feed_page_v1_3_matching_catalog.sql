-- get_feed_page v1.3 — total_matching_catalog (FASE 2 fixes, follow-up #5)
--
-- Bug (15/06): a exaustão do feed mostrava "filtros muito restritivos" (B) em
-- vez de "você viu as relevantes" (A) depois de swipar todas as relevantes —
-- porque total_after_filters/total_available são PÓS-swipe, então "esgotou"
-- e "filtros zeram tudo" ficavam indistinguíveis. O legacy já foi corrigido
-- (totalMatchingCatalog no JobRepository); aqui o RPC ganha o mesmo sinal.
--
-- total_matching_catalog = nº de vagas ativas (não expiradas) que BATEM com os
-- filtros IGNORANDO swipe. >0 = havia relevantes (esgotou → A); 0 = filtros
-- restritivos (→ B). Retornado só na 1ª página (e na sentinela), como os
-- outros totais.
--
-- Mudança estrutural vs v1.2: `base` passa a incluir as swipadas (flag
-- is_swiped); os conjuntos distintos de área/cidade saem do catálogo INTEIRO
-- (corrige o caso "swipou todas as vagas da área" — a área sumia de areas_d e
-- o match zerava); `scored` filtra `not is_swiped` pro feed; `matching_catalog`
-- conta sobre tudo. Resultado das rows do feed = IDÊNTICO à v1.2 (paridade
-- intocada) — só acrescenta a coluna de contagem.
--
-- Adicionar coluna ao RETURNS TABLE exige DROP + CREATE (CREATE OR REPLACE não
-- troca o tipo de retorno). Grants re-aplicados após o CREATE.

drop function if exists public.get_feed_page(
  int, numeric, uuid, text[], text[], text[], text[], int, timestamptz);
...
begin
  ...
  matching_catalog as (
    ...
    where sa.pass_area and sa.pass_model and sa.pass_type and sa.pass_loc and sa.pass_salary
  ),
